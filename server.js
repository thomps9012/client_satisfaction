...
app.post("/api/begin", async (req, res) => {
    ...
    res.json(db_res.insertedId)
})
app.post("/api/question", async (req, res) => {
    const { interview_id, question_id, client_answer } = req.body
    await client.connect();
    const survey_collection = client.db(db_name).collection('surveys')
    const db_res = survey_collection.findOneAndUpdate({ _id: interview_id }, {
        $push: {
            answers: {
...
app.post("/api/end", async (req, res) => {
    ...
    const only_answered = record.answers.filter(({ client_answer }) => client_answer != -1)
    const client_score = only_answered.reduce((a, b) => a + b.client_answer, 0)
    const db_res = survey_collection.findOneAndUpdate({ _id: interview_id }, { $set: { total_score: client_score, applicable_questions: only_answered.length } })
    res.json(db_res)
})
